...
export function exportTasksToExcel(tasks = [], reportTitle = 'CT University Executive Task & NAAC Audit Report') {
  ...
  const exportData = tasks.map((t) => {
    ...
    return {
      ...
      'Assigned By': t.creatorName || 'Admin',
...
export function exportTasksToPDF(tasks = [], reportTitle = 'NAAC & NBA Accreditation Audit Report', reportType = 'Accreditation Inspection') {
  ...
  const getStageClass = (stage) => {
    const s = stage || '';
    if (s === 'Accepted') return 'st-completed';
    if (s.includes('Review') || s === 'Re-issued') return 'st-review';
    return 'st-progress';
  };

  const deptRowsHtml = Object.entries(deptMap).map(([dept, stat]) => {
  ...
  const taskRowsHtml = tasks.map(t => `<tr>
    <td><strong>${t.id}</strong></td>
    <td>${t.title}</td>
    <td>${t.assigneeName || 'Faculty'}</td>
    <td>${t.creatorName || 'Admin'}</td>
    <td>${t.departmentName || t.dept || 'Engineering'}</td>
    <td>${t.priority || 'Medium'}</td>
    <td><span class="status-pill ${getStageClass(t.stage)}">${t.stage || 'Assigned'}</span></td>
...
<body>
  <div class="no-print" style="background:#eff6ff;border:1px solid #bfdbfe;padding:12px 16px;border-radius:8px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center;">
    <span style="font-weight:700;color:#1e40af;font-size:13px;">📄 Official CT University Audit Report — Ready for Print / PDF Export</span>
    <button onclick="window.print()" style="padding:8px 16px;background:#2563eb;color:#fff;border:none;border-radius:6px;font-weight:700;cursor:pointer;font-size:12px;">🖨️ Print / Save as PDF</button>
  </div>
  ...
  <div class="meta-grid">
    ...
    <div class="meta-card">
      ...
      <div class="meta-lbl">Accepted / Completed</div>
    </div>
    <div class="meta-card">
    ...
    <div class="meta-card">
      ...
      <div class="meta-lbl">Completion Velocity</div>
    </div>
  </div>
